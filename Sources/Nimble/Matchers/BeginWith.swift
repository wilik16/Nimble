...
public func beginWith<S: Sequence>(_ startingElement: S.Element) -> NimblePredicate<S> where S.Element: Equatable {
    return NimblePredicate.simple("begin with <\(startingElement)>") { actualExpression in
        guard let actualValue = try actualExpression.evaluate() else { return .fail }

        var actualGenerator = actualValue.makeIterator()
        return NimblePredicateStatus(bool: actualGenerator.next() == startingElement)
    }
}

/// A Nimble matcher that succeeds when the actual collection's first element
/// is equal to the expected object.
public func beginWith(_ startingElement: Any) -> NimblePredicate<NMBOrderedCollection> {
    return NimblePredicate.simple("begin with <\(startingElement)>") { actualExpression in
        ...
        #endif
        return NimblePredicateStatus(bool: collectionValue.isEqual(startingElement))
    }
}
...
public func beginWith(_ startingSubstring: String) -> NimblePredicate<String> {
    return NimblePredicate.simple("begin with <\(startingSubstring)>") { actualExpression in
        guard let actual = try actualExpression.evaluate() else { return .fail }

        return NimblePredicateStatus(bool: actual.hasPrefix(startingSubstring))
    }
}
