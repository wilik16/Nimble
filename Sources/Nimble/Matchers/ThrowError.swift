...
/// that parameter.
public func throwError<Out>() -> NimblePredicate<Out> {
...
/// nil arguments indicates that the matcher should not attempt to match against
/// that parameter.
public func throwError<T: Error, Out>(_ error: T, closure: ((Error) -> Void)? = nil) -> NimblePredicate<Out> {
    return NimblePredicate { actualExpression in
        var actualError: Error?
        do {
            _ = try actualExpression.evaluate()
        ...
        return NimblePredicateResult(bool: matches, message: message)
    }
}
...
/// nil arguments indicates that the matcher should not attempt to match against
/// that parameter.
public func throwError<T: Error & Equatable, Out>(_ error: T, closure: ((T) -> Void)? = nil) -> NimblePredicate<Out> {
    return NimblePredicate { actualExpression in
        var actualError: Error?
        do {
            _ = try actualExpression.evaluate()
...
public func throwError<Out>(closure: @escaping ((Error) -> Void)) -> NimblePredicate<Out> {
    return NimblePredicate { actualExpression in
        var actualError: Error?
        do {
            _ = try actualExpression.evaluate()
...
///
/// The closure only gets called when an error was thrown.
public func throwError<T: Error, Out>(closure: @escaping ((T) -> Void)) -> NimblePredicate<Out> {
    return NimblePredicate { actualExpression in
        var actualError: Error?
        do {
            _ = try actualExpression.evaluate()
